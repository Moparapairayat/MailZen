...
<head>
    ...
    <title>{{ __('Forgot Password') }} - {{ config('app.name', 'MailZen') }}</title>
...
<body class="font-sans antialiased bg-gray-50">
    <div class="min-h-screen flex items-center justify-center px-4 py-12">
        <div class="w-full max-w-md bg-white rounded-xl border border-gray-200 shadow-sm p-8">
            ...
            <p class="mt-2 text-sm text-gray-600">{{ __('Enter your email and we will send you a password reset link.') }}</p>

            @if(session('status'))
                <div class="mt-6 rounded-md bg-green-50 p-4 border border-green-200">
