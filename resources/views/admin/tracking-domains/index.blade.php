...
@section('title', __('Tracking Domains'))
@section('page-title', __('Tracking Domains'))
...
<div class="space-y-6">
    <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between lg:gap-0">
        <div class="w-full lg:flex-1 lg:max-w-lg">
            <form method="GET" action="{{ route('admin.tracking-domains.index') }}" class="flex flex-col gap-2 lg:flex-row lg:items-center">
                <input
                    type="text"
                    name="search"
        ...
        <div class="flex items-center justify-end">
            <x-button href="{{ route('admin.tracking-domains.create') }}" variant="primary" class="w-full lg:w-auto">{{ __('Add Domain') }}</x-button>
        </div>
    </div>
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex items-center justify-end gap-2">
                                    <x-button href="{{ route('admin.tracking-domains.show', $domain) }}" variant="table" size="action" :pill="true">{{ __('View') }}</x-button>
                                    <x-button href="{{ route('admin.tracking-domains.edit', $domain) }}" variant="table" size="action" :pill="true">{{ __('Edit') }}</x-button>
                                    <form method="POST" action="{{ route('admin.tracking-domains.destroy', $domain) }}" class="inline" onsubmit="return confirm('{{ __('Are you sure?') }}');">
                                        @csrf
                                        @method('DELETE')
